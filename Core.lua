-- [[ 🌚刘某某脚本🌝 | 完美白边集成版 (含V2.9核心) | Core.lua ]]
...
V29Btn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
...
V29Btn.MouseButton1Click:Connect(function()
    ShowConfirm("刘某某 V2.9", function()
        ...
        local _CG2 = game:GetService("CoreGui")
        ...
        if _CG2:FindFirstChild("LMM_Final_V36") then _CG2.LMM_Final_V36:Destroy() end
        local _S = Instance.new("ScreenGui", _CG2); _S.Name = "LMM_Final_V36"; _S.ResetOnSpawn = false
        ...
        local _Mini = Instance.new("TextButton", _S)
        _Mini.Name = "MiniButton"
        _Mini.Size = UDim2.new(0, 65, 0, 65)
        _Mini.Position = _M.Position
        _Mini.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
        _Mini.Visible = false
        _Mini.Text = "" 
        _Mini.AutoButtonColor = false
        _Mini.ZIndex = 999
        Instance.new("UICorner", _Mini).CornerRadius = UDim.new(1, 0)

        local _MStroke = Instance.new("UIStroke", _Mini)
        _MStroke.Thickness = 3
        _MStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

        _RS.Heartbeat:Connect(function()
            _Glow.Color = _RGB_CORE.Color
            _MStroke.Color = _RGB_CORE.Color
        end)

        local _mDragging, _mStart, _mPos, _pressTime
        _Mini.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                _mDragging = true
                _pressTime = tick()
                _mStart = input.Position
                _mPos = _Mini.Position
        ...
        _UIS.InputChanged:Connect(function(input)
            if _mDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                local delta = input.Position - _mStart
                _Mini.Position = UDim2.new(_mPos.X.Scale, _mPos.X.Offset + delta.X, _mPos.Y.Scale, _mPos.Y.Offset + delta.Y)
            end
        end)

        _Mini.MouseButton1Up:Connect(function()
            if tick() - _pressTime < 0.2 then
                _M.Position = _Mini.Position
                _M.Visible = true
                _Mini.Visible = false
            end
        end)
        ...
        local _Title2 = Instance.new("TextLabel", _TB2)
        _Title2.Size = UDim2.new(1, 0, 1, 0); _Title2.Text = "🌚刘某某脚本🌝"; _Title2.Font = "GothamBold"; _Title2.TextSize = 18; _Title2.TextColor3 = Color3.new(1, 1, 1); _Title2.BackgroundTransparency = 1

        local _SF2 = Instance.new("ScrollingFrame", _M)
        _SF2.Size = UDim2.new(1, 0, 1, -70); _SF2.Position = UDim2.new(0, 0, 0, 60)
        _SF2.BackgroundTransparency = 1; _SF2.ScrollBarThickness = 12 
        _SF2.AutomaticCanvasSize = Enum.AutomaticSize.Y; _SF2.CanvasSize = UDim2.new(0, 0, 1.25, 0); _SF2.Active = true
                ...
                if p ~= _LP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if _G_LMM_88.v_esp_box then
                        ...
                        if bb.Name ~= "LMM_BOX" then 
                            bb.Name = "LMM_BOX"; bb.AlwaysOnTop = true; bb.Size = UDim2.new(4.5, 0, 6, 0); bb.Adornee = tHrp; 
                            local f = Instance.new("Frame", bb); f.Size = UDim2.new(1,0,1,0); f.BackgroundTransparency = 1; 
                            local st = Instance.new("UIStroke", f); st.Name = "S"; st.Thickness = 2 
                        end
        ...
        local function _Ctrl(t, x, c, f)
            local b = Instance.new("TextButton", _TB2); b.Size = UDim2.new(0, 32, 0, 32); b.Position = UDim2.new(1, x, 0.5, -16); b.Text = t; b.BackgroundColor3 = c; b.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", b); b.MouseButton1Click:Connect(f)
        end
